...
\end{aligned}
$$
However, the model description provided above is purely theoretical; in practice, it is impossible to construct a mixture model with infinite components. Thus, we implement a finite approximation of the model. We let $\pi_k$, $\lambda_k$ and $K$ be defined as earlier. In addition, we explicitly define the distributions of $\alpha$ and $\mathbb{G}_0$ describing the underlying Dirichlet Process $\text{DP}(\alpha \mathbb{G}_0)$. The full Bayesian model is as follows:
$$
\begin{aligned} 
...
\vspace{-12mm}
\begin{center}
\includegraphics[width=0.32\textwidth]{diagram.PNG}  
\vspace{-8mm}
\hspace{15mm}
...
While the categorical distribution could select directly from $\lambda_k$, the cluster-based approach allows the algorithm to utilize information on the assignments $z_i$, which can be applied in nonparametric clustering methods such as those discussed in [@Zhang2019DPMM].





<!-- which will be utilized be used as the `MixingDistribution` object in the DP sampler  package [@dirichletprocess2023] to produce a posterior distribution. In  -->
...
The Gibbs sampling procedure on $10,000$ iterations with a burn-in of $B =100$ had a total run time of $703.87$ seconds, and converged to the posterior rates and distribution consistently across multiple runs and randomization seeds. In addition, we ran the Poisson DPMM on synthetic data, and observed consistent recovery of source weights in the mean results and posterior weights. Furthermore, the synthetic data of equivalent size to the experiment data had a similar run time of $732.53$ seconds, implying that the chain is mixing well.  In the table below, we report the posterior rate estimates and the associated mixing weights for the Chicago Police data set.
...
```{r}
# An appropriate combination of diagnostics, synthetic datasets and other validation strategies.
# synthetic: report rate recovery of synthetic data and runtime
# MLEs under the false assumption of homogeneous rates 
```
